...
public class RegistroEntradaController implements Initializable {
    ...
    private void onGuardarEntrada() {
        try {
            ...
            // Asignación segura del ID de usuario para el movimiento de bodega
            int idUsuarioBodega = 2; 
            try {
                // Si tu clase SesionUsuario cuenta con un método específico para el ID, puedes integrarlo aquí.
                // Ejemplo: idUsuarioBodega = SesionUsuario.getInstancia().getIdUsuario();
            } catch (Exception ex) {
                System.err.println("Aviso: Usando ID de usuario por defecto (2).");
            }
